...
subprocess.check_call(['pip', 'install', 'demoji'])
...
def streamy_login(driver) -> bool:
    while True:
        try:
            ...
            time.sleep(3)
            if driver.current_url != 'https://streamyme.com/wp-admin/':
...
def new_post(driver, title, content) -> bool:
    try:
        ...
        # action.perform()
        # print(driver.find_element(By.ID, 'mceu_0-open').text)
        ...
        while True:
            try:
                ...
                pub = driver.find_element(By.ID, 'publish')
    ...
    while True:
        ...
        for message in messages:
            ...
            if update_id > last_telegram_id:
                try:
                    content = demoji.replace(message['channel_post']['text'], "")
                    while '\n\n' in content:
                    ...
                    if new_post(driver, title, content) is True:
                        ...
                        print(f'--> New post have published.')
                    else:
                        print(f'--> Failed to publish new post.')
                except Exception as e:
                    continue
