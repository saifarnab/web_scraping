...
def send_message_to_slack(text, color='good'):
    ...
    SLACK_WEBHOOK = 'https://hooks.slack.com/services/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'
...
def send_email_via_google(sender_email, sender_pass, receiver_email, receiver_name, reply_to: str) -> str:
    try:
        ...
        raw_template = f"Hi {receiver_name.split(' ')[0]}, I am curious if you have considered outsourcing your customer" \
                       ...
                       f"than a toupee in a hurricane".replace(
            "\'", '').replace("\"", '').strip()
...
def add_activity_note(api, lead_id: str, note: str):
    ...
    logging.info("Note added to Close")
...
def run():
                ...
                if connected_account[4]:
                    ...
                    if template != "":
                        ...
                        create_email_sent_confirmation(conn, sender_email, receiver_email, False,
                                                       date.today().strftime('%Y-%m-%d %H:%M:%S'), str(template),
                                                       reply_to)
                        ...
                        logging.info(
                            f'--> Failed to send email {receiver_email} from {sender_email} via Gmail API')
    ...
    # send_message_to_slack(
    # f'Script has executed successfully! Total {success_counter} emails have been sent out.')
