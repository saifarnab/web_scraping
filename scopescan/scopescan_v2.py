...
def is_duplicate(entity: str, address: str) -> bool:
    excel_file = "output.xlsx"
    df = pd.read_excel(excel_file)
    df = df[["Entity", "Address"]].values
    if [entity, address] in df:
        print('true')
        return True
...
def extract_data(driver: webdriver.Chrome):
    ...
    """['Spark Digital Capital', 'Jump trading', 'Galaxy Digital', 'New Form Capital', 'Geeq', 'Robert Leshner',
            ...
            for counter in range(11):
                try:
                    ...
                    svg = driver.find_element(By.XPATH,
                                              "//td[@class='ant-table-cell ant-table-cell-row-hover']//span[@aria-label='copy']//*[name()='svg']")
                    svg.click()
                    time.sleep(2)
                    address_text = pyperclip.paste()
                    ...
                    # if is_duplicate(entity, address_text) is False:
