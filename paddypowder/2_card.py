...
def make_unique_id(unique_list:list):
    return ''.format(unique_list)
...
def search_keyword(driver, keyword) -> list:
    ...
    tag = wait.until(EC.presence_of_element_located((By.TAG_NAME, "html")))
    for _ in range(40):
        tag.send_keys(Keys.ARROW_DOWN)

    # elements = driver.find_elements(By.XPATH, "//*[@class='list__item']")
    # elements = driver.find_elements(By.XPATH, "//*[@class='abc-list__item']")
...
def get_data_over_under(driver,url):
        ...
        try:
            ...
            for u in data:
                ...
                if "Over 3.5 Cards" in d and "Cards Over/Under 3.5" in title:
                    d = ' '.join([line.strip() for line in d.strip().splitlines() if line.strip()]).replace('Cards', '')
                    d = d.split(' ')
                    try:
                        temp = round(eval(d[-1]),2)
                    except:
                        temp = d[-1]
                    d.pop(-1)
                    # d = ' '.join(d) + " : " + temp
    ...
    if cnd and len(text)>3:
        ...
        # unq_id = make_unique_id([url,time,tournament,match,text])
        sending_telegram(message)
        # return message,unq_id
        # cnd = False
...
def sending_telegram(message):
    ...
    # print('msg sent')
    token = '5194166895:AAGO9MBn7gtvnunoAMkMC9qGKhAvnEM72MA'
    chat_id = '-1001513987186'
...
def main():
    print('Getting Data..')
    # get_data_over_under(driver, 'https://www.paddypower.com/football/romanian-liga-i/arges-pitesti-v-universitatea-craiova-31370655?tab=corners-cards')
    # print('done')
    unique_id = set()
    while True:
...
if __name__ == '__main__':
    main()
    # sending_telegram('My name is khan')